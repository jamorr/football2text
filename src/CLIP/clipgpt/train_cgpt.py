...
from transformers import GPT2Tokenizer, GPT2LMHeadModel, AdamW, get_linear_schedule_with_warmup,ViTImageProcessor
...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
          lr: float = 2e-5, warmup_steps: int = 5000, output_dir: str = ".", output_prefix: str = "",collate_fn = None):
    ...
    train_dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True,collate_fn=collate_fn)
...
def main():
    ...
    # exit()
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='/media/jj_data/data/CLIP/train/text_image.parquet')
    parser.add_argument('--out_dir', default='/media/jj_data/models/GPT/checkpoints')
    parser.add_argument('--prefix', default='football_prefix', help='prefix for saved filenames')
...
#----------------------------------------------------------------------------------------------------------------------------------#
    ...
    def tokenize_captions(examples):
        ...
        text_inputs = gpt2tokenizer(captions,
            max_length=args.max_seq_length,
            padding="max_length",
            truncation=True,)
...
#----------------------------------------------------------------------------------------------
    ...
    if args.only_prefix:
        ...
        sys.stdout.flush()
    train(train_dataset, model, args, output_dir=args.out_dir, output_prefix=args.prefix,collate_fn=collate_fn)
